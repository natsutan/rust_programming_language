...
}, combinator::{opt, recognize}, multi::{fold_many0, many0}, number::complete::recognize_float, sequence::{delimited, pair}, Parser, IResult,
error::ParseError};
...
imple_op_from!(LoadLiteral, Store, Copy, Add, Sub, Mul, Div, Call, Jmp, Jf, Lt, Pop);
...
impl Compiler {
    ...
    fn add_copy_inst(&mut self, stack_idx: StkIdx) -> InstPtr {
        let inst = self.add_inst(OpCode::Copy, (self.target_stack.len() - stack_idx.0 - 1) as u8,);
    ...
    fn add_pop_util_inst(&mut self, stack_idx: StkIdx) -> Option<InstPtr> {
        if self.target_stack.len() <= stack_idx.0 {
            return None;
        }
        let inst = self.add_inst(
            OpCode::Pop, (self.target_stack.len() - stack_idx.0 - 1) as u8,
        );
    ...
    fn compile_expr(&mut self, ex: &Expression) -> StkIdx {
        match ex {
            ...
            Expression::FnInvoke(name, args) => {
                ...
                let args = args.iter().map(|arg| self.compile_expr(arg)).collect::<Vec<_>>();
...
enum Expression<'src> {
    ...
    If(Box<Expression<'src>>, Box<Expression<'src>>, Option<Box<Expression<'src>>>),

}

fn write_program(source: &str, writer: &mut impl Write, out_file: &str, disasm: bool) -> std::io::Result<()>{
...
fn if_expr(i: &str) -> IResult<&str, Expression> {
    ...
    Ok((
        i,
        Expression::If(
            Box::new(cond), Box::new(t_case), f_case.map(Box::new),
        )
        ))
}
